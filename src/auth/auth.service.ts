...
export class AuthService {
  ...
  async signup({ username, email, password }: AuthCredentialDto): Promise<any> {
    ...
    if (exitUser) {
      // user exist but not yet verify due to otp expired
      if (exitUser.status === '2') {
        let newOtp = crypto.randomInt(10000, 99999).toString();

        const optStore = new OtpStore();
        optStore.otp = newOtp;
        optStore.userId = exitUser.id;
        await this.optRepository.save(optStore);

        await this.mailerService.sendVerificationEmail(exitUser.email, newOtp);

        return {
          token: exitUser.verification_token,
          message: 'new signup verify code has send to email',
        };
      }
    ...
    // generate random otp 5 digit number
    ...
    try {
      ...
      return {
        token: vToken,
        message: 'signup verify code has send to email',
      };
    } catch (err) {
      await queryRunner.rollbackTransaction();
    } finally {
  ...
  async findUser({ token, otp }: OtpDto): Promise<any> {
    ...
    if (user.status === '1') {
      throw new ConflictException(`${user.username} already verify`);
    }

    const foundLatest = await this.optRepository
    ...
    let myExpiredOtp = moment(foundLatest.created_at)
      .add(10, 'minutes')
      .toDate();
  ...
  async findEmail({ email }: EmailDto) {
    ...
    return {
      token: newToken,
      message: 'reset password verification link has send',
    };
  }
